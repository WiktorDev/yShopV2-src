...
exports.makePayment = (secret, notyficationPass, shopID, productID, name, price, uuid, nick, req, res, next)=>{
    const hpay = new Payment.HotPay(secret, notyficationPass);
    if(req.body.count){
        totalPrice = price*req.body.count
    }else{
        totalPrice = price;
    }
    hpay.generatePayment(parseFloat(totalPrice), name, config.baseURL+"/payment/redirect/"+uuid, uuid).then((data)=>{
        if(JSON.parse(data).URL == null){
            ...
            product.getProduct(productID, function(pr){
                ...
                if(req.body.count){
                    ...
                    payment.savePayment(shopID, uuid, 'hotpay', productID,req.body.count, false, nick)
                }else{
                    ...
                    payment.savePayment(shopID, uuid, 'hotpay', productID,1, false, nick)
                }
                res.render('status')
            })
        }
...
exports.setSecret = (shopID, secret)=>{
    db.query("UPDATE `yshop_payment_gateway` SET `hotpay_secret`='"+secret+"' WHERE `shopID`='"+shopID+"'")
}

exports.setNotyficationPass = (shopID, notyfication)=>{
    db.query("UPDATE `yshop_payment_gateway` SET `hotpay_notification`='"+notyfication+"' WHERE `shopID`='"+shopID+"'")
}

exports.setName = (shopID, name)=>{
    db.query("UPDATE `yshop_payment_gateway` SET `hotpay_name`='"+name+"' WHERE `shopID`='"+shopID+"'")
}
